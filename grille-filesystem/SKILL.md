...
- `fs_read_file` — Read file contents. Supports UTF-8 and UTF-16 LE/BE. Paginate large files with `offset_bytes` and `max_bytes`.
- `fs_write_file` — Write a file atomically (temp-then-rename). Default refuses to overwrite; set `overwrite: true` to replace.
- `fs_str_replace` — Surgically replace a unique string in a file. Enforces uniqueness — fails if the string appears zero or more than once. The hero tool for targeted edits.
...
- **Do not use `fs_str_replace` when `old_str` spans multiple lines in a Windows file with CRLF line endings.** CRLF breaks multi-line matching. Use a unique single-line anchor (e.g. a section header or the last distinct line) that falls within the replacement target.
